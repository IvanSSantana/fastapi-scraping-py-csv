# 📊 Relatório Financeiro Simplificado — {{TICKER}}

Data: {{DATA_ATUAL}}

---

## 🧾 Visão Geral

Empresa/FII do segmento **{{SEGMENTO}}**, com foco em {{DESCRICAO_RESUMIDA}}.

---

## 📈 Indicadores Fundamentais

#TODO: Configurar um LLM para fazer somente a interpretação
| Indicador | Valor | Interpretação |
|----------|------|--------------|
| P/L | {{PL}} | {{PL_ANALISE}} |
| P/VP | {{PVP}} | {{PVP_ANALISE}} |
| Dividend Yield | {{DY}} | {{DY_ANALISE}} |
| ROE | {{ROE}} | {{ROE_ANALISE}} |
| ROIC | {{ROIC}} | {{ROIC_ANALISE}} |

---

## 🧠 Interpretação dos Indicadores

TODO: Fazer uma IA específica somente para ler o CSV ou StockResponse dos indicadores e dizer se estão bons ou ruins
{{INTERPRETACAO_INDICADORES}}

---

## 🗂️ Eventos Corporativos Recentes

#TODO: Organizar resposta da IA para retornar lista de jsons com campos data e descrição
{{EVENTOS}}
- **{{DATA}}** — {{DESCRICAO}}
{{/EVENTOS}}

---

## ⚠️ Impactos Potenciais

{{ANALISE_IA}}

---

## 📊 Conclusão / Resumo

{{CONCLUSAO}}

---

*Este relatório foi gerado automaticamente com base em dados públicos e análise assistida por IA.*